;(function ($) {
	...
	const createMessageBtn = () => {
		...
		let lang = locationArr.find(_ => _ === 'ru') ? 'ru' : 'uk'
		locationArr = locationArr.filter(_ => _ !== lang)
		if (locationArr[locationArr.length - 1].includes('ru.html')) {
			lang = 'ru'
		} else if (locationArr[locationArr.length - 1].includes('uk.html')) {
			lang = 'uk'
		} else {
			lang = lang
		}
		locationArr.pop()
		var locationSource = locationArr.join('/')
		messageBtn.setAttribute('href', locationSource + `/${lang}/contacts.html`)
	}

	document.addEventListener('DOMContentLoaded', createMessageBtn)

	$(document).ready(function () {
		let body = document.querySelector('body')

		let messageBtn = document.createElement('a')
		messageBtn.setAttribute('id', 'messageBtn')
		messageBtn.style =
			'position: fixed; z-index: 2147483647; bottom: 5; right: 20'

		let messageIcon = document.createElement('i')
		messageIcon.classList.add('fa')
		messageIcon.classList.add('fa-envelope')

		body.appendChild(messageBtn)
		...
		============================*/
		// $.scrollUp({
		// 	scrollText: '<i class="fa fa-envelope"></i>',
		// 	easingType: 'linear',
		// 	scrollSpeed: 900,
		// 	animation: 'fade',
		// })
		...
		;(function () {
			...
			$(window).scroll(function () {
				if (120 < $(window).scrollTop() && !scrolled) {
					nav
						.addClass('sticky_menu animated fadeInDown')
